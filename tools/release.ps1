# Builds a complete snapory release — the portable self-contained exe and the
# Windows installer — both named with the version read from snapory.csproj.
#
#   .\tools\release.ps1                   -> artifacts land in dist\release
#   .\tools\release.ps1 -OutDir <folder>  -> also copied into <folder>
#
# The -OutDir hook lets the outer workspace collect the build into builds\snapory
...
$project = Join-Path $root 'snapory\snapory.csproj'
...
Write-Output "Building snapory v$version"
...
$portable = Join-Path $releaseDir "snapory-v$version.exe"
$setup    = Join-Path $releaseDir "snapory-setup-v$version.exe"
Copy-Item (Join-Path $selfContainedDir 'snapory.exe') $portable -Force
Copy-Item (Join-Path $root "dist\installer\snapory-setup-v$version.exe") $setup -Force
